...
void setup() {
  ...
  Serial.println("timestamp_ms,letter_shown,button_pressed,correct_press,vibration_event,vibration_duration_ms,vibration_count,progress,score,time_left_ms");
...
void loop() {
  ...
  if (lastButtonState == HIGH && buttonState == LOW) {
    ...
    logEvent(now, true, correct, timeLeft);
  }
  ...
  if (vibrationDetected && lastVibrationState == HIGH) {
    vibrationStart = now;
    vibrationCount++;
    logEvent(now, false, false, timeLeft);
  }
...
void logEvent(unsigned long timestamp, bool buttonPressed, bool correct, unsigned long timeLeft) {
